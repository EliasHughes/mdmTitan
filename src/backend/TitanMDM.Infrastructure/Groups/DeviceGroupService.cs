    ...
    : IDeviceGroupService
{
    ...
    public async Task<IReadOnlyCollection<DeviceGroupDto>>
        GetAllAsync(
            Guid organizationId,
            CancellationToken cancellationToken = default)
    {
        ...
        var counts =
            await _dbContext.DeviceGroupMembers
                .AsNoTracking()
                .Where(x =>
                    x.OrganizationId == organizationId)
                .GroupBy(x => x.GroupId)
    ...
    public async Task<DeviceGroupDetailsDto?>
        GetByIdAsync(
            Guid organizationId,
            Guid groupId,
            CancellationToken cancellationToken = default)
    {
        ...
        var members =
            await (
                from member in
                    _dbContext.DeviceGroupMembers
                        .AsNoTracking()

                join device in
                    _dbContext.Devices
                        .AsNoTracking()
                    on member.DeviceId equals device.Id

                where
                    member.GroupId == deviceGroup.Id &&
                ...
                select new DeviceGroupMemberDto(
                    device.Id,
                    device.DeviceName,
        ...
        var existingDeviceIds =
            await _dbContext.DeviceGroupMembers
                ...
                .Where(x =>
                    x.OrganizationId == organizationId &&
                    x.GroupId == groupId &&
                    validDevices.Contains(x.DeviceId))
                .Select(x => x.DeviceId)
                .ToListAsync(cancellationToken);

        foreach (
            var deviceId in
            validDevices.Except(existingDeviceIds))
        {
            _dbContext.DeviceGroupMembers.Add(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _dbContext.DeviceGroupMembers.Remove(
            member);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _dbContext.DeviceGroups.Remove(
            deviceGroup);
